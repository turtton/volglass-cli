...
fun Path.isDirectory(): Result<Boolean> =
    kotlin.runCatching {
        fs.metadata(this).isDirectory
    }

fun Path.list(): Result<List<Path>> =
    kotlin.runCatching {
        fs.list(this)
    }

fun Path.extractFiles(): Result<Unit> =
    list().map { result ->
        result.forEach {
...
fun Path.copy(targetPath: Path): Result<Unit> =
    kotlin.runCatching {
...
fun Path.copyRecursively(
    targetPath: Path,
    ignoreRule: Regex? = null,
): Result<Unit> =
    kotlin.runCatching {
        if (ignoreRule != null && targetPath.name.contains(ignoreRule)) {
            return@runCatching
        }
...
fun Path.atomicMove(targetPath: Path): Result<Unit> =
    kotlin.runCatching {
